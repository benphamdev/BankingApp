...
public class UserSpecificationBuilder {
    ...
    public UserSpecificationBuilder with(
            String isOrPredicate, String key, String operation, Object value, String prefix, String suffix
    ) {
        Enums.SearchOperation searchOperation = Enums.SearchOperation.getSimpleOperation(operation.charAt(0));
        if (searchOperation == Enums.SearchOperation.EQUALITY) {
            boolean startWithAsterisk = prefix != null && prefix.contains(Enums.SearchOperation.ZERO_OR_MORE_REGEX);
            boolean endWithAsterisk = suffix != null && suffix.contains(Enums.SearchOperation.ZERO_OR_MORE_REGEX);
